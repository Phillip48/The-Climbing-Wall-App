...
// Call server function in appwrite to compress the video
const callAppwriteFunctionVideoCompress = async (data = {}) => {
  try {
    const response = await functions.createExecution(
      "672cfe89002e1bda9a66", // function id
      JSON.stringify(data) // Send data as a JSON string
    );
...
export async function resetPassword(form) {
  try {
    const sendReset = await account.createRecovery(
      form.email,
      "http://localhost/new-pass" // redirect link to a page - What goes here?
    );
...
// Get all sends
export async function findUserEmail(form) {
  try {
    // Find user by email
    const posts = await databases.listDocuments(
      appwriteConfig.databaseId,
      appwriteConfig.userCollectionId,
      [Query.equal("email", form.email), Query.limit(5000)]
    );
    let userId = posts.documents[0].$id;
    console.log(userId);
    // Update user password
    const passwordUpdate = await account.updatePassword(userId, form.password, [
      // Permission.read(Role.user(userId)),
      // Permission.update(Role.user(userId)),
      // Permission.read(Role.guests()),
      // Permission.update(Role.guests()),
      // Permission.read(Role.guests(userId)),
      // Permission.update(Role.guests(userId)),
    ]);
...
export async function uploadFile(file, type) {
  ...
  let videoCompression = await callAppwriteFunctionVideoCompress({
    data: asset,
  });
  try {
    ...
    // console.log("Original Video Obj ", asset);
    // console.log("Compressed Blob ", videoCompression);
    // console.log("Create File ", uploadedFile);
...
// Get send posts that matches search query
export async function searchSendsTitle(query) {
  // console.log(query);
  try {
    const posts = await databases.listDocuments(
      appwriteConfig.databaseId,
